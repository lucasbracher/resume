...
\textit{Programming Languages} - Python, PHP, Golang, C, C++, C\#, JavaScript, Clojure, Golang, VBA, Delphi\\
\textit{Databases} - PostgreSQL, PostGIS, MySQL, MongoDB\\
\textit{Test} - pytest, Selenium\\
\textit{Operating Systems} - Windows, Linux, MacOS\\
\textit{Other} - AWS (Lambda, Rekognition, SageMaker, API Gateway, S3, RDS), LLMs (AzureAI, OpenAI), ElasticSearch, pdflatex, LaTeX, OpenCV, Scrapy, Bash, Docker, RabbitMQ, Celery, git, .NET Compact\\
}
